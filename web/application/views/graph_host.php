...
<?php
// var graphs = ['num_loaded_processes'];
echo "var graphs = [";
foreach ($graphs as $g) {
    echo "'$g', ";
}
...
$('#datepicker').datepicker({
    ...
    showWeek: true,
    firstDay: 1,
    onSelect: function(dateText, inst) { basetime = (parseInt(dateText) / 1000).toString(); draw_hosts_graphs();}
});
...
function draw_graphs(cmpt_id, label) {
    for (x in graphs) {
        ...
        var json_url = site_url + '/ajax/json/' + component + '/' + g + '/' + period + '/' + cmpt_id + '/' + lb;
        if (basetime) {
            json_url += '/' + basetime;
        }
        $.ajax({url: json_url,
...
function draw_hosts_graphs() {
data = [];
<?php
    foreach ($hosts as $h) {
        $hid = $h->id;
        $label = $h->ip_name;
        echo "draw_graphs('$hid', '$label');\n";
    }
?>
}
